...
\begin{itemize}
\item Write a simple lattice code to evaluate a two-particle correlation function as a function of Euclidean time using a $Z_2$ auxiliary field. Simple sources for two-particle states may be delta functions in position or momentum space. Use the long time behavior of the effective mass function, $\ln \frac{C_2(\tau)}{C_2(\tau+1)} \tautoinfty E_0$ (see \Sec{observables}), to determine the ground state energy for your choice of coupling, $g$. Compare this with what you expect from \Eq{eigeqlambda}, using the relation $\lambda = e^{-E_0}$, as the number of lattice points is increased.

\item Much of these lecture notes follow this review: arXiv:1208.6556. There you will also find more information about algorithms. The following is an excellent pedagogical introduction to EFT's by David B. Kaplan: arXiv:nucl-th/0510023.

\item Construct sources for three fermions in an $l=0$, $l=1$ state and find the lowest energies corresponding to each state at unitarity. Which $l$ corresponds to the true ground state of this system?

\item Explore the cumulant expansion using a toy model \cite{EKLN2}: 
\beq
