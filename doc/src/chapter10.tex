...
\author{Heiko Hergert, Scott K.~Bogner, Justin G.~Lietz, Titus D.~Morris, Samuel J.~Novario, Nathan M.~Parzuchowski, and Fei Yuan}
\institute{
...
% 
Scott K.~Bogner  \at Department of Physics and Astronomy and National Superconducting Cyclotron Laboratory, Michigan State University, East Lansing, Michigan USA, \email{bogner@nscl.msu.edu}, \and 
% 
...
% 
Titus D.~Morris \at Department of Physics and Astronomy, University of Tennessee, Knoxville, Tennessee, USA,
and Physics Division, Oak Ridge National Laboratory, Oak Ridge, Tennessee, USA, \email{titusmorris@gmail.com}, \and 
% 
Samuel J.~Novario \at Department of Physics and Astronomy and National Superconducting Cyclotron Laboratory, Michigan State University, East Lansing, Michigan,  USA, \email{novarios@nscl.msu.edu},\and 
% 
Nathan M.~Parzuchowski  \at Department of Physics and Astronomy and National Superconducting Cyclotron Laboratory, Michigan State University, East Lansing, Michigan USA, \email{parzuchowski@frib.msu.edu}, \and 
% 
...
transformations to evolve the nuclear Hamiltonian to a desired shape. In the
IMSRG, the nuclear ground state is decoupled from all excitations in order to solve 
the many-body Schr\"odinger equation. We discuss the IMSRG formalism 
...
theory (Chap.~\ref{chapter:cc}), Configuration-Interaction Monte Carlo 
(Chap.~\ref{chapter:qmc}), and the Self-Consistent Green's Function approach discussed 
in Chap.~\ref{chapter:scgf}. The chapter concludes with an expanded overview of 
...
in Chaps.~\ref{chapter:cc}, \ref{chapter:qmc}, and \ref{chapter:scgf} 
...
(Sec.~\ref{sec:current_mrimsrg}).  Third, we will discuss applications of
...
\begin{table}[t]
  \begin{center}
      ...
      \end{tabular*}
  \end{center}
  \caption{\label{tab:srg_pairing_sp}Single-particle states and their quantum numbers and their energies from Eq.~\eqref{eq:def_pairing_hamiltonian}. The degeneracy for every quantum number $p$ is equal to 2 due to the two possible spin values.} 
\end{table}
...
beyond-HF methods in Chap.~\ref{chapter:cc}. In second quantization, the 
...
into even smaller sub blocks. As in Chap.~\ref{chapter:cc}, we 
consider the $S_z=0$
...
interactions (see Chap.~\ref{chapter:cc}). However, we quickly realize the
...
\begin{align}\label{eq:opflow_NN}
  \totd{}{s}\vO^{[2]}_{ij} &= \comm{\etaO}{\vO^{[2]}_{ij}}\,.
\end{align}
...
in earlier chapters of this book. An example is the toy problem discussed in Sec.~ 
\ref{sec:srg_toy}: We actually expanded our $2\times2$ matrix in terms of the algebra 
...
to face the issue of induced many-body forces, as discussed in Sec.~ 
\ref{sec:srg_induced}.
...
Chap.~\ref{chapter:cc}, \cite{Epelbaum:2009ve,Machleidt:2011bh}). AV18
...
Lanczos algorithm (see Chap.~\ref{chapter:cc} and \cite{Lanczos:1950sp})
...
Fig.~\ref{fig:schematic}(a) by using a generator like \cite{Anderson:2008hx}
...
by evolving $\lambda\to0$, just like we evolved $s\to\infty$ in Secs.~\ref{sec:srg_toy}
...
the SRG generator at the two-body level (\emph{N$N$-only}). The energy
...
the matrix flow in the $A-$body system (cf.~Sec.~\ref{sec:srg_nn}),
...
been evolved to a low resolution scale $\lambdaSRG$ (see Sec.~ 
...
etc.) for the number of contractions in a term (cf.~Chap.~\ref{chapter:cc}).
...
the computational expense of explicitly treating three-body operators. In Sec.~ 
...
(see Chaps.~\ref{chapter:cc} and \ref{chapter:scgf}). Since all three
...
(see Secs.~\ref{sec:imsrg_pairing} and \ref{sec:imsrg_neutron_matter} 
...
as CCSD and ADC(3) (see Chaps.~\ref{chapter:cc} and \ref{chapter:scgf}).
...
want to keep, and off-diagonal parts we want to suppress (cf.~Sec.~ 
...
avoid inducing strong in-medium $3N, \ldots$ interactions to maintain the
...
Chap.~\ref{chapter:qmc}, \cite{Carlson:2015lq} and references therein). Explicitly 
...
code that is designed for solving the pairing Hamiltonian (see Sec.~ 
...
codes in Secs.~\ref{sec:srg_pairing_implementation} and \ref{sec:srg_nn_flow}. It extracts
...
level spacing constant at $\delta=1.0$. As we see in the left panel, 
...
(CCD) --- see Chap.~\ref{chapter:cc} for details. We see that the correlation
...
single-particle states, see Tab.~\ref{tab:srg_pairing_sp}) and $6-g$
...
This reflects our findings for the pairing Hamiltonian, where we 
...
is prohibitively expensive, we are forced to solve Eq.~\eqref{eq:obsflow} 
...
meets certain conditions (see \cite{Magnus:1954xy}):
...
In the multireference case, we choose a suitable correlated reference 
...
multireference CC methods (see, e.g., \cite{Lyakh:2012zr}), for us it only
...
by inspecting the oxygen charge radii (see Fig.~\ref{fig:OXX}). 
...
general. The charge radii grow larger as $\lambdaSRG$ decreases,
which is consistent with a study for light nuclei 
...
particle ($q$) orbitals (see Fig.~\ref{fig:valence_space})\footnote{In
...
(cf.~Sec.~\ref{sec:imsrg_decoupling}). This only leaves diagram (IV), 
...
\end{equation}. Multiplying Eq.~\eqref{eq:eom} by $\UO(\infty)$ and recalling that 
\begin{equation}
...
tool for pre-processing nuclear interactions and operators, leading to
...
renormalization group techniques, and the reason why we consider the IMSRG 
...
\end{equation}
(see Sec.~\ref{sec:imsrg} and Chap.~\ref{chapter:cc}).
