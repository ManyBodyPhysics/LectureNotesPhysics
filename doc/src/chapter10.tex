...
forms an eigenbasis for the non-interacting Hamiltonian $\hat{H}_0$ and we have used the discrete completeness relation $\sum{k=0}^{\infty}\vert k \rangle \langle k \vert = 1$. 
...
Returning to our pairing problem, the input parameters to the Fortran program {\em pairing.f90} is easily compiled with any available Fortran compiler. The reader needs simply to specify the input parameters to the pairing model in the file {\em pairing.ini}, displayed here as well
\begin{svgraybox}
\begin{lstlisting}
