...
\caption{Second-order interaction-irreducible contributions to the self-energy arising from both two- and three-nucleon forces. The diagram depending on the effective two-body interactions (left) also shows the indices and labels that are used for calculating its contribution in example 11.1. }
\label{fig:2ndOrd}
\end{center}
\end{figure}
...
\vskip .3 cm
\noindent
{\bf Example 11.1.} Calculate the Feynman-Galitskii propagator, $G^{II,f}(\tau)$, that c\
orresponds to the propagation of two particles or two holes that do not interact with ea\
ch other.
...
\end{align}
  ...
  \begin{prob} \label{problem:prob11.9} 
