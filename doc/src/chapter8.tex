...
As computational  scientists we work at the intersection of our own scientific
domain, mathematic and computer science. Most of us have a deep formal education
in the first two subjects, but are not well versed in the computer science
aspects of our work. For many of us, the software engeneeing aspects of our work
has been thought of as something you pick up along the way and not worth
spending efforts on during our formal education. Of course, there are exceptions
but this chapter is not written for them.
There is a rich scientific literature and textbooks dedicated to the topic of
software engineering. There are many philosophies and schools of thoughts on how
to write software and what programming paradigms are best suited for particular
jobs. You could get lost in the literature trying to find the one true way. In
this book we take a more pragmatic approach and present a few key strategies to
make our jobs a little bit easier and ensure that the results we present are
correct.
Most of us don't write scientific software as part of big teams or for
customers, but are rather solo writers that seek to answer a scientific
question. As such, our priorities are somewhat different than a team of
developers writing software for a paying client. Therefore, we rarely need the
strict and formal ways of approaching code that are necessary in large teams.
However, we will try to capture the essence of these philosophies and extract
those components that are useful and necessary in our own work.
As writers of scientific software, our main concern in that our results are
correct. We will focus on the tools and strategies that we use to ensure
correctness, but also on tools and strategies to avoid losing work and as our
projects grow, a development style that encourages many small independent parts
that can be tested separately and reused in other projects and continuous
...
writing tests. The only difference is when you think about testing from the
...
\end{table}
    ...
    The profile in Listing XXX shows that the calls to the Minnesota potential
    is still the most time-consuming function, but the number of times this
    function is called seems too high. Indeed, we see that we call the potential
...
{Chapter8-programs/cpp/MBPT/mbpt3V05.cpp}
    ...
    have also replaced the explicit summation with matrix products.
...
\end{table}
    ...
    shows the profile. We have reduced the number of calls to the potential
    ...
    functions in the profile. Listing XXX shows the new profile. While the
    profiler now seems a little bit confused about which functions are being
    called, it is clear that almost 80\% of the total run time is spent in some
    version of an exponential function. To get more information than this we
    would have to compile the standard c libraries with the -pg option also, but
    we have enough information to continue. Listing XXX shows the relevant parts
    of the minnesota potential code. The exponential function is called 6 times
    for each matrix element. By using the exponent rules, we can rewrite the
        exponential as a product of multiple exponential functions that is
        composed of factors of exp(q1x*q2x), exp(q1y*q2y), and exp(q1z*q2z). By
        pre-computing these exponential functions we can reduce the number of
        calls to the exponential function from around 400 million in our test
        case to around 7 million. That should take a big chunk out of the run
        time for this program. We leave this as an exercise for the reader.
        ...
        have shown in Listing XXX. This will enable multiple execution streams
        ...
        these directives. Table XXX shows the total run time for different
...
of MBPT3 with OpenMP is shown in Listing \ref{code:mbpt3V04} and Figure XXX shows the
