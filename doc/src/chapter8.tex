...
example
Refs.~\cite{binder2013,hergert2013,roth2012,hagen2012a,hagen2012b,cipollone2013,hebeler2010b,carbone2013}.
...
\end{lstlisting}
  ...
  \[
    ...
    \frac{1}{2}\sum_{i\le F}^A\sum_{j \le F}^A\left\langle ij\vert
      \hat{v}\vert ij \rangle-\langle-\sum_{i\le F}^A \langle i\vert\hat{u}^{HF}\vert i\rangle.
  \]
...
Eqs.~(\ref{eq:bruteforceMBPT}) and (\ref{eq:smartMBPT}) more efficient. For pedagocical reasons, we will use the MBPT2 part of the program
due to its simplicity. 
The codes can be found at the github link 
\url{https://github.com/ManyBodyPhysics/LectureNotesPhysics/tree/master/doc/src/Chapter8-programs/cpp/MBPT2/src}.
We start by discussing the basic structure of these codes by looking at the main program. A shortened version is presented here. The full version can be obtained by accessing the link below.   In the listing here we display only the two functions which compute
the above two equations. The first function performs a standard matrix-matrix multiplication
\begin{equation}
\hat{A}=\hat{B}\hat{C}   \Longrightarrow a_{ij} = \sum_{k=1}^{n} b_{ik}c_{kj},
\label{eq:mtxmtx}
\end{equation}
written as 
\lstset{language=c++}
...
\end{lstlisting}
while the second function implements the brute force formula with sums running over all possible single-particle states. 
\begin{lstlisting}
...
double MBPT2corr(TB_OpChannels &vnn, int numChannels, abstractSPbasis * SPbasis)
{
  ...
  for(int ichan = 0; ichan < numChannels; ichan++){    
	...
	corr += vnn.hhpp[ichan].getElement(hhIndex,ppIndex)*
                vnn.hhpp[ichan].getElement(hhIndex,ppIndex)/energyDenom;	
      }
    }
  }
  corr = 0.25*corr;
...
double mbpt2_full_hhpp(double * vnn_full_hhpp, abstractSPbasis * SPbasis)
{
  ...
  for(int i=0; i<fermiLevel; i++){
	...
	for(int b=0; b<NparticleStates; b++){	
	  ...
	  corr += vnn_full_hhpp[hhpp_index(i,j,a,b,SPbasis)]*
                  vnn_full_hhpp[hhpp_index(i,j,a,b,SPbasis)]/energyDenom;
	}
      }
    }
  }
...
In order to analyze our results we present here two sets of calculations for  neutron matter with density $n=1.0$ fm$^{-3}$. 
We use 20 major shells to define our effective Hilbert space, amounting thus to $921$ plane wave single-particle states with periodic boundary conditions. The first calculation employing  Eqs.~(\ref{eq:bruteforceMBPT}) and (\ref{eq:smartMBPT}) is obtained using $N=14$ neutrons  to define the Fermi level. The time below is in seconds.
\begin{svgraybox}
\begin{verbatim}}
./main.exe 1 1.0 1 20 14
mass*c^2: 939.565MeV
hbar^2/(2m) = 20.7213MeV*fm^2
L = 2.41014fm V= 14fm^3
k_fermi 3.093667726280136fm^-1
SPBasis:
i nx ny nz sz tz E
0 0 0 0 -1 -1 0
1 0 0 0 1 -1 0
2 -1 0 0 -1 -1 140.8284783563085
3 -1 0 0 1 -1 140.8284783563085
....
....
921 3 3 2 1 -1 2848.528061166434

CHANNELS
NumChannels: 14739
Two Body Channels setup time: 0.1424539089202881
Block matrix Load time: 0.3275527954101562
Block matrix mult time: 0.003997087478637695
Total Block calc time: 0.4740951061248779
MBPT2corr/A = -2.391665703622324

full matrix load time: 8.278779029846191
full struct mult time: 3.508285999298096
Total full struct calc time: 11.78709602355957
corrMBPT2_full/a = -2.3916657036224
...
\begin{svgraybox}
\begin{verbatim}
./main.exe 1 1.0 1 20 38
mass*c^2: 939.565MeV
hbar^2/(2m) = 20.7213MeV*fm^2
L = 3.36198fm V= 38fm^3
k_fermi 3.093667726280136fm^-1
SPBasis:
i nx ny nz sz tz E
0 0 0 0 -1 -1 0
1 0 0 0 1 -1 0
2 -1 0 0 -1 -1 72.37470049053114
3 -1 0 0 1 -1 72.37470049053114
....
....
921 3 3 2 1 -1 1342.557267007174

CHANNELS
NumChannels: 14739
Two Body Channels setup time: 0.1530601978302002
Block matrix Load time: 1.729802846908569
Block matrix mult time: 0.01892781257629395
Total Block calc time: 1.901870012283325
MBPT2corr/A = -1.461098136683107

full matrix load time: 60.36804699897766
full struct mult time: 33.2056519985199
Total full struct calc time: 93.573725938797
corrMBPT2_full/a = -1.461098136683009
\end{verbatim}
\end{svgraybox}
Both calculations were performed on a MacBook Pro laptop with a 3GHz Intel Core i7 and an 8GB 1600 MHx DDR3 memory.  Due to memory limitations, larger calculations cannot be performed for  Eq.~(\ref{eq:bruteforceMBPT}) on a standard laptop. 
There are several important observations to be made here. For Eq.~(\ref{eq:bruteforceMBPT}) almost $2/3$ of the total time is spent setting up and loading the full matrix. The summation over the individual single-particle levels accounts for the remaining $1/3$ of the time. Using the decomposition into two-body channels in Eq.~(\ref{eq:smartMBPT}), the largest fraction of the computations are spent on loading the matrix elements, while the matrix-matrix multiplication (implemented in the simplest possible way) accounts for less than $1\%$. This time increases however when we move from $N=14$ to $N=38$. 
For Eq.~(\ref{eq:smartMBPT}) we can easily run much larger systems, as seen here for $N=114$ and $30$ and $40$ shells. For $N=30$ we have  
\begin{svgraybox}
\begin{verbatim}
./main.exe 1 1.0 1 30 114
....
1789 5 3 1 1 -1 968.2347088363201

CHANNELS
NumChannels: 27783
Two Body Channels setup time: 0.5911049842834473
Block matrix Load time: 25.70273399353027
Block matrix mult time: 0.2907769680023193
Total Block calc time: 26.5846860408783
MBPT2corr/A = -1.899977657908343
\end{verbatim}
\end{svgraybox}
For $40$ shells we obtain
\begin{svgraybox}
\begin{verbatim}
./main.exe 1 1.0 1 40 114
....
2713 6 3 1 1 -1 1350.847359528065

CHANNELS
NumChannels: 46875
Two Body Channels setup time: 1.432629823684692
Block matrix Load time: 43.99419498443604
Block matrix mult time: 0.4821138381958008
Total Block calc time: 45.90901184082031
MBPT2corr/A = -1.900268075095771
\end{verbatim}
\end{svgraybox}
First we note that the MBPT2 contribution to the correlation energy is
more or less converged with $40$ shells. Most of the computational
time, more than $95\%$, is spent on loading the block decomposed
matrix elements. Even with $N=114$ single-particle states and $40$
major shells, the matrix-matrix multiplication is a mere $1\%$ of the
total time.  Increasing to $100$ major shells, with $10^4$
single-particle states and $N=114$ neutrons defining the Fermi level
barely changes the time spent on the matrix-matrix multiplication. For
$100$ major shells, more time is now spent on setting up the
Hamiltonian matrix. In our codes we set up the Hamiltonian matrix
before performing the computations. The total time spent on loading
the block-structured Hamiltonian for $10^4$ single-particle states is now reduced to $82\%$ of the total
time.  Increasing to $150$ shells, or $2\times 10^4$ basis states, the percentage of time spent on loading matrix elements for the matrix-matrix multiplication is now reduced to $71\%$. 
This has important consequences for efficient implementations
of coupled cluster theory. As we demonstrated above, the CCD equations
can all be rewritten in terms of matrix-matrix multiplications. The
above matrix-matrix multiplication accounts only for a tiny fraction
of the computational time expenditure. Even with efficient
matrix-matrix multiplication algorithms like Strassen's algorithm
\cite{strassen} and optimized parallel matrix-matrix implementations,
most of the time is actually spent and loading matrices and setting up the
Hamiltonian matrices.  For coupled cluster calculations this has
important consequences. It means that developing efficient parallel
algorithms for setting up, storing and fetching matrix elements becomes
increasingly important.  With increasing number of particles and single-particle states, one may even consider computing matrix elements on the fly.

We discuss now different  high-performance strategies, limiting ourselves to the basic matrix-matrix multiplication of MBPT2. 
The simplest possible extension to our algorithm is to implement efficient algorithms for matrix-matrix multiplications. 
In our code above we have implemented the matrix-matrix multiplication using a straightforward implementation, fetching each matrix elements when needed. Furthermore, we include an explicit evaluation of the energy denominators as well. A first possible improvement would be to replace the smarter algorithm with efficient matrix-matrix and/or matrix-vector multiplication functions from for example BLAS \cite{blas}. 
In order to do so, it is convenient to set up the matrices as follows (here we use {\em armadillo} \cite{armadillo} for declaring and setting up matrices)
\begin{lstlisting}
// calculate the 2nd Order Many-Body Perturbation Theory correlation energy 
double MBPT2MatrixMatrix(int numChannels)
{
  double corr = 0.;  
  for(int ichan = 0; ichan < numChannels; ichan++){    
    int hh = getHoleConfigs(ichan);
    int pp = getParticleConfigs(ichan);
    mat V(hh,pp), T(pp,hh);
    //  Here we fetch the interaction matrix V and the matrix T that includes the 
   //   interaction and the energy denominator
    FetchMatrices(ichan, V, T);
    for(int hhIndex = 0; hhIndex < hh; hhIndex++){
	corr += sum(V(hhIndex,:)*T(:,hhIndex);
      }
    }
  }
  corr = 0.25*corr;
