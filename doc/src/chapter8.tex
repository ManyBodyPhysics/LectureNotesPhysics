...
that can run on Linux, OSX, and Windows. By tracking changes, it's easier to
...
GitLab~(\url{https://about.gitlab.com/}), which uses git to track all changes to the code. By
...
complicated calculation.  The codes can be found at the github link
...
guide our development efforts. Our starting point are naiive implementations of
MBPT2 and MBPT3 diagrams that take too long to run. For reference, we calculate
...
Listing \ref{code:mbpt2V06} shows a possible early implementation to solve eq.
\ref{eq:bruteforceMBPT} from MBPT2. This function has a loop over all single-particle indices and calls the
...
We want to decrease the run time of this application, but it can be difficult to
decide where we should spend our time improving this code. Our first order of
business is to observe what goes on inside the program. For that we will use a
profiling tool called gprof~(\url{https://sourceware.org/binutils/docs/gprof/})
...
\% is spent in the loops in the actual MBPT2 function. The remaining couple of
percent is spent in the spinExchangeTerm function which is called from the
...
nuclear interaction we know that most of these calls result in a matrix element
of zero. If we can exploit this structure to reduce the number of calls to the
...
not called when we know ithe matrix element is zero. This code loops over
...
using matrix products by calling the blas dgemm wrappers \verb;RM_dgemm; and
...
The profile is now alot more busy and it shows a longer runtime
...
calls to the exponential function in the potential code. If we can reduce the
...
cores are fighting eachother for access to memory and cache, which reduces
performance somewhat. Any further optimization of this code would yield
diminishing returns
