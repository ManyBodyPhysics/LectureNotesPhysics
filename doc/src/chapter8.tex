...
\subsection{Models from Efective field theory for the two- and three-nucleon interactions}\label{subsec:forcemodels}
...
%\vspace*{-0.25cm}
\scalebox{0.14}{\includegraphics{Figs/diagNNLO.pdf}}
%\vspace*{-1.0cm}
...
Define a general onebody operator $\hat{F} = \sum_{i}^A\hat{f}(x_{i})$ and a general  twobody operator $\hat{G}=\sum_{i>j}^A\hat{g}(x_{i},x_{j})$ with $g$ being invariant under the interchange of the coordinates of particles $i$ and $j$. 
\begin{enumerate}
\item[a)]
\[
