...
\begin{figure}[hbtp]
    \includegraphics[width=0.7\linewidth,angle=90]{Chapter8-figures/mbpt.pdf}
      \caption{Linked anti-symmetrized Goldstone diagrams which enter the definition of the ground-state
...
The standard method for solving these equations is to
  ...
  \[
  (t_{ij}^{ab})^{(0)}=\frac{\langle ab \vert \hat{v} \vert ij
  ...
  \[
  \Delta E_{\mathrm{CCD}}^{(0)}=\frac{1}{4}\sum_{abij}\langle
    ij \vert \hat{v} \vert ab \rangle(t_{ij}^{ab})^{(0)}=\frac{1}{4}\sum_{abij} \frac{\langle
  ...
  \label{eq:ccd1}
  \end{equation}
In the above and following equations we have dropped the subscript which indicates the number of iterations. 
  ...
  index, can be rewritten as  a matrix inversion problem.  Alternatively, the same equation can be solved by iteration. Solving
  ...
  solution of Eq.~(\ref{eq:ccd1}), is thus to
  \begin{enumerate}
...
Independent of your scientific background, you are probably also educated in
...
physical experiment. The codes we develop are the blueprints the compiler uses to build
...
build, but also by humans to understand what is being built, how to fix
...
V\_Minnesota function via the get\_element function to calculate the two-body interaction for each set of
...
focus on high-performance computing aspects and code efficiency. This will mimick our discussion of many-body perturbation theory, in particular the calculation of the correlation energy to second order discussed above. 
...
<<<<<<< HEAD
while MBPT3 includes diagrams (3), (4), (5), (8) and (9) as well. Note that diagram (3) is zero for the pairing model. With a canonical Hartree-Fock basis, as studied in the examples below, all diagrams with so-called Hartree-Fock insertions disappear. The MBPT calculations, as well as the CCD calculations have all been done with a canonical Hartree-Fock basis. For the pairing model the single-particle energies implify to those in Table \ref{tab:pairingmodelsp}. 
=======
while MBPT3 includes diagrams (3), (4), (5), (8) and (9) as well. Note that diagram (3) is zero for the pairing model and that diagrams (8) and (9) do not contribute either if we work in the canonical Hartree-Fock basis.
>>>>>>> origin/master
  In the case of the simple pairing model it is easy to calculate
  $\Delta E_{MBPT2}$ analytically. This is a very useful  check of our codes since this analytical expression  can  also be used to check our first CCD iteration.
...
state. The python program linked to above is included mainly for pedagogical reasons. As 
we have already seen, this approach is extremely inefficient from a
computational point of view. In our discussions of MBPT, as well as for CCD code, we rewrite
...
four-particle-four-hole correlations.  However, the disagreement for
...
many-body perturbation theory to third order.  
Coupled cluster theory with doubles only shows a very good
...
however observe larger discrepancies.
  \begin{figure}
    ...
    \caption{Correlation energy for the pairing model with exact diagonalization, CCD and perturbation theory to third order (MBPT3) for a range of interaction values.}
...
(again the average of ten numerical experiments), resulting in a considerable speed up.
It is useful to dissect the final time in terms of different
...
Hamiltonian,
  ...
  \end{equation}
  For a finite basis, as discussed earlier, we approximate the problem by constructing a box
  with sides of length $L$, which quantizes the momentum, and impose
  periodic boundary conditions in each direction by requiring 
...
a fixed number of neutrons $N=114$. 
  \begin{figure}
    ...
    \label{fig:fig2}
  \end{figure}
We see from Fig.~\ref{fig:fig2} that at the CCD level and neutron matter only there is a good convergence with  $\mathrm{N_{max}=25}$.
These results depends however on the type of interaction and many-body approximation. 
