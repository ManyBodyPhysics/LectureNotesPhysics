...
If we do not define a core, this defines normally what is
  ...
  Hartree-Fock calculation or just the eigenstates of the one-body operator that defined the
...
(see section \ref{sec:mbpt}) to first order in the interaction is given by
  ...
  C_{k}^{c}+\sum_{cdkl}\langle \Phi_{ij}^{ab} | \hat{H} -E|\Phi_{kl}^{cd}
...
For larger dimensionalities $d$, with $d > 10^5$, iterative Krylov methods \cite{krylov} like Lanczos' \cite{lancszo} or Davidson's \cite{davidson} 
algorithms are frequently used. These methods yield, with a finite number of iteration, only a subset of all eigenvalues of interest. Lanczos' algorithm converges to the extreme values, yileding the lowest-lying and highest-lying eigenstates, see for example Ref.~\cite{golubvanloan} for a proof. 
...
With the eigenvectors we can compute
  ...
  \sum_{abij}\langle ij | \hat{v}| ab \rangle C_{ij}^{ab}.
  \]
The energy of for example the ground state is then
  \[
  ...
  \]
  The state $\vert \Psi_0\rangle$ is not normalized, rather we employ again
  the intermediate normalization $\langle \Phi_0 \vert
  \Psi_0\rangle=1$.
...
and the total energy
  ...
  \Phi_0\rangle.
  \]
  In Rayleigh-Schr\"odinger (RS) perturbation theory we set $\omega = W_0$ and obtain the
  ...
  In the full configuration interaction theory  section we showed that we could rewrite the exact state
  ...
  $G$-matrix, see subsection \ref{subsec:gmatrix}. 


In summary, many-body perturbation theory introduces order-by-order specific correlations and we
can make comparisons with exact calculations like those provided by configuration interaction theory.
The advantage of for example Rayleigh-Schr\"odinger perturbation theory is that 
at every order in the interaction, we know how to  calculate all contributions since they
can be either tabulated or computed on the fly.
However, many-body perturbation theory suffers from not being variational theory and  there is no guarantee
that higher orders will improve the order-by-order convergence.   It is also extremely tedious to compute terms beyond third order, in particular
if one is interested in effective valence space interactions.  
There are however classes of correlations which can be summed up to infinite order in the interaction. 
The hope is that such correlations can mitigate specific convergence issues, although there is no a priori guarantee thereof. 
Examples are the so-called TDA and RPA class of diagrams \cite{ripkablaizot}, as well as the resummation of two-particle-two-hole correlations. If we limit ourselves to the resummation of two-particle correlations only, these leads us to the so-called $G$-matrix resummation of diagrams discussed in the next subsection. There are however computationall inexpensive methods which sum larger classes of correlations to infinite order in the interaction. This leads us to section \ref{sec:cctheory} and the final  many-body method of this chapter, coupled cluster theory. Before we proceed however, we will digress into a discussion of the so-called $G$-matrix resummation of two-particle correlations  since it provides a useful bridge between many-body perturbation theory and coupled cluster theory.
  ...
  Brueckner \cite{bruekcner1956}, and further developed by Goldstone and Bethe, Brandow and
  Petschek \cite{bps1964}.  In the literature it is generally referred to as the
  ...
  \begin{equation}
        \hat{H}\Psi_0(A)=E_0(A)\Psi_0(A),
  \end{equation}
  with $\hat{H}=\hat{T}+\hat{V}$ where $A$ denotes the number of nucleons, $\hat{T}$ is the
  kinetic energy and $\hat{V}$ is the nucleon-nucleon (NN) potential.
  The corresponding unperturbed problem is
  \begin{equation}
        \hat{H}_0\Phi_0(A)=W_0(A)\Phi_0(A).
  \end{equation}
Since we are interested in nuclear matter only, $\hat{H}_0$ is just kinetic energy $\hat{T}$ and $\Phi_0$ is a Slater
  ...
  just $-E_0/A$. In passing, we remind the reader that binding energy per partcile for infinite 
  ...
  how it is used in calculations. We will also assume the reader is familiar 
with the standard rules for setting up the analytical expressions for various Feymann-Goldstone diagrams.
\begin{figure}
    \includegraphics[width=\linewidth]{Chapter8-figures/mbpt2p.pdf}
      ...
      interaction $V$, while diagrams (ii) and (iii) are examples of
       contributions to second and third order in the interaction, respectively.}
      \label{fig:goldstone}
\end{figure}

  Using the standard diagram rules, the various
  ...
  \end{equation}
  with $n_h=n_l=2$ and $n_{ep}=1$. The quantity $n_h$
  ...
  \begin{equation}
        G_{pqrs}=V_{pqrs} +\sum_{ab>0}V_{pqab}\frac{Q(ab)} {\omega
          -\varepsilon_a-\varepsilon_b} G_{abrs},
        \label{eq:gwithq}
  \end{equation}
  ...
  Eq. (\ref{eq:ggeneral}) we have a restricted sum over $ab$, while in
  ...
  different from that in Eq. (\ref{eq:gwithq}).  If we include also two-hole states, we may resum
two-particle-two-hole diagrams to infinite order, see for example Ref.~\cite{dickhoff}.
