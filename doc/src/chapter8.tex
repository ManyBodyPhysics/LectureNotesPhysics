...
\url{https://github.com/ManyBodyPhysics/LectureNotesPhysics/tree/master/doc/src/Chapter8-programs/}.
...
guide our development efforts. Our starting point are naive implementations of
...
the first implementation in a way which is as close as possible to mathematical expression, here Eq.~\ref{eq:bruteforceMBPT}.
\begin{table}
...
to say that we can't reach our goals with this code.
...
code. Our first order of business is to observe what goes on inside
...
code. We use the Minnesota potential for testing and benchmark purposes only and
it will not be used for production runs. It is, however, possible to reduce the
...
errors significantly. It's a good thing we have tests to make sure that the
results haven't changed between versions. Table \ref{profile:mbpt3V05} shows the
profile for this version and we have reduced the number of calls the potential
...
discussions aboe we used repeatedly the simple pairing model of
