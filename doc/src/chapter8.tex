...
the time to start. There are several tools available~\cite{refneeded}, but the
authors are using git~\cite{refneeded}, an open-source version-control system
...
GitLab~\cite{refneeded}, which uses git to track all changes to the code. By
...
In this section we will demonstrate the use of a simple profiler to help
...
using the Minnesota potential discussed in section \ref{refneeded}. As the model
is not as important as the performance in this section, we postpne a discussion
...
\ref{refneeded} from MBPT2. This function has a loop over all single-particle indices and calls the
...
profiling tool called gprof~\cite{refneeded}.
...
OpenMP~\cite{refneeded} directives. This will split the work between multiple
